...
\begin{figure}
	\begin{tikzpicture}[
		...
		node[left=10pt] {Inmatning(ar)} (x3.south west);
	\end{tikzpicture}
...
biasen, $b$, ger den weighted input som är $\Sigma_nw_nx_n + b$ och
är ekvivalent med $z$.
För att sedan få utmatningen används en aktiveringsfunktion på den weightade
inputen. En sigmoidfunktion:

\begin{equation} \label{eq:sigmoid}
	\sigma(z) \equiv \frac{1}{1 + e^{-z}} \\
\end{equation}

%lite om att de nbllir mättad med stora tal
är ett exempel på en aktiveringsfunktion, vilken ger utmatningen
$\sigma(%\Sigma_nw_nx_n + b)$. -- se vilken av dessa borde det vara?
z)$.
...
Den här arkitekturen kallas \textit{feedforward}, inforamtion matas
alltid framåt mellan neuroner.
Medan det är förhållandevis rakt på sak att bestämma antalet in- och
utmatningsneuroner, så finns ingen generell regel för hur många gömda lagren
och antalet neuroner dessa ska innehålla. Istället får alltså t.ex. antalet
neuroner ställas i förhållande till hur lång tid det tar att träna nätet.
Med andra ord är neuronnät specifika för en uppgift och en arkitektur som
fungerar bra i ett fall behöver nödvändigtvis inte göra det i ett annat.
