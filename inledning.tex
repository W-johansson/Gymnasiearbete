...
Friktionskraftens beräknas med:
\begin{equation}
	F_f = \lVert F \rVert \cdot r \cdot \mu
\end{equation}
där $F$ är kraftens storlek, $r$ är radien, och $\mu$ friktionskeofficienten
som finns mellan bromsskivan och bromsbeläggen vilken kan variera mellan 0,2 - 0,4.
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[legend style={at={(0.02,0.98)},anchor=north west},
			...
			\addplot[YellowOrange] {(1)/(1+e^-x)};
			\addplot[RubineRed] {(1)/(1+e^(-2*x+4))};
			\addplot[NavyBlue] {(1)/(1+e^(-0.5*x-1))};
		\end{axis}
	\end{tikzpicture}
